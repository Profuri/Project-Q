...
namespace Fabgrid
{
    public class AddTile : State
    {
        ...
        private void SetupControls()
        {
            ...
            tileOffsetProperty = serializedTilemap.FindProperty("newTile.offset");
        ...
        private void AddSingleTile()
        {
            if (tilemap.newTile.prefab != null)
            {
                ...
                {
                    var bounds = FabgridUtility.GetTileWorldBounds(tilemap.newTile.prefab, tilemap.newTile.sizeCalculationOption, tilemap.newTile);
                    var newTile = CreateTile(tilemap.newTile.prefab, bounds, tilemap);
        ...
        private void AddMultipleTiles()
        {
            ...
            for (int i = 0; i < prefabPaths.Length; ++i)
            {
                ...
                if (asset != null)
                {
                    ...
                    {
                        var bounds = FabgridUtility.GetTileWorldBounds(asset, tilemap.newTile.sizeCalculationOption, tilemap.newTile);
                        var newTile = CreateTile(asset, bounds, tilemap);
                        AddTileIfNotDuplicate(newTile);
                    }
                }
            }
        ...
        private static Tile CreateTile(GameObject prefab, Bounds bounds, Tilemap3D tilemap)
        {
            return new Tile
            {
                prefab = prefab,
